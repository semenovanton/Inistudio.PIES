...
namespace Inostudio.Test.Core
{
    ...
    public class FlushingBuffer<TValue>
        : IFlushingBuffer<TValue>
    {
        ...
        public FlushingBuffer(string fileName, int bufferSize)
        {
            FileName = fileName;
        ...
        {
            //push value into concurrent dict.
            _bag[value] = false;
        ...
        protected bool TryGetNext(out TValue value)
        {
            ...
            if (_bag.Count > 0)
            {
                var minKey = _bag.Where(x => !x.Value).Min(x => x.Key);
        ...
        /// </summary>
        protected ConcurrentDictionary<TValue, bool> _bag = new  ConcurrentDictionary<TValue, bool>();
        #endregion
